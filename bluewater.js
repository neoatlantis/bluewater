	...
	__nest__ (
		...
		'ui.session', {
			__all__: {
				__inited__: false,
				__init__: function (__all__) {
					var __name__ = 'ui.session';
					var UISession = __class__ ('UISession', [object], {
						__module__: __name__,
						get __init__ () {return __get__ (this, function (self, session) {
							self.DOM = (function __lambda__ () {
								return $ ('#session');
							});
							self.session = session;
							self.__initUI ();
						...
						get __initUI () {return __get__ (this, function (self) {
							...
							ui.start ('#firebase-login', dict ({'callbacks': dict ({'signInSuccessWithAuthResult': (function __lambda__ () {
								return false;
							}), 'signInSuccess': (function __lambda__ () {
								return false;
							})}), 'signInOptions': list ([firebase.auth.GoogleAuthProvider.PROVIDER_ID, firebase.auth.GithubAuthProvider.PROVIDER_ID])}));
						});}
					});
					__pragma__ ('<all>')
						__all__.UISession = UISession;
